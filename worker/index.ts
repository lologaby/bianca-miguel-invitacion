...
const invitation: InvitationPayload = {
  ...
  event: {
    couple: { first: 'Bianca', second: 'Placeholder' },
    ...
    ceremony: {
      name: 'Iglesia Cristiana Discípulos de Cristo',
...
function hasSession(request: Request) {
  ...
  return cookie.split(';').some((part) => part.trim() === `${cookieName}=accepted`);
}
...
async function api(request: Request, env: Env) {
  ...
  if (url.pathname === '/api/guest' && request.method === 'POST') {
    ...
    const valid = body?.code === previewCode || body?.linkToken === previewLink;
    if (!valid) return json({ error: 'invalid_invitation' }, { status: 401 });
    return json(invitation, {
      headers: {
        'set-cookie': `${cookieName}=accepted; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=28800`,
      },
    });
  }

  if (url.pathname === '/api/session' && request.method === 'GET') {
    return hasSession(request) ? json(invitation) : json({ error: 'unauthorized' }, { status: 401 });
  }
  ...
  if (url.pathname === '/api/rsvp' && request.method === 'POST') {
    ...
    await env.DB.prepare(`
      INSERT INTO rsvps (guest_id, guest_name, attendance, party_size, plus_one_name, song, dietary, accessibility, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).bind(
      ...
      body.attendance,
      partySize,
      clean(body.plusOneName, 80),
      clean(body.song, 120),
      clean(body.dietary, 300),
      clean(body.accessibility, 300),
