...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    const responses = keys.length ? await redis.mget<StoredRsvp>(...keys) : [];
    ...
    return res.status(503).json(jsonError('store_unavailable', 'Las confirmaciones no están disponibles. Inténtalo de nuevo.'));
  }
}
