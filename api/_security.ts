...
export interface ApiRequest { cookies: Record<string, string | undefined>; headers: Record<string, string | string[] | undefined>; body?: Record<string, unknown>; query?: Record<string, string | string[] | undefined>; method?: string }
...
export function readSession(req: ApiRequest): SessionGuest | null {
  try {
    const raw = req.cookies.bianca_session;
    if (!raw) return null;
    ...
    const guest = JSON.parse(Buffer.from(payload, 'base64url').toString()) as SessionGuest;
    if (!guest || typeof guest.id !== 'string' || !guest.id || typeof guest.name !== 'string'
      || !Number.isInteger(guest.partyLimit) || guest.partyLimit < 1
      || typeof guest.plusOneAllowed !== 'boolean' || !Number.isFinite(guest.exp)) return null;
    return guest.exp > Date.now() ? guest : null;
  } catch {
    return null;
  }
}
